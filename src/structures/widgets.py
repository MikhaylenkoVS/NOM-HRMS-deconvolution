...
from src.ui import BG, FG, ACCENT, PANEL, BTN, WARN, FONT, MONO, IMG_W, IMG_H
...
class StructureDialog(tk.Toplevel):
    ...
    def _build(self, info: dict, rdmol):
        ...
        formula_str = "  ".join(
            f"{k}{v}" for k, v in sorted(info.get("formula", {}).items())
        )
        tk.Label(self, text=formula_str, bg=BG, fg=FG, font=FONT).pack()
        tk.Label(
            self,
        ...
        if rdmol is not None and PIL_OK:
            ...
            if pil:
                ...
                tk.Label(self, image=self._tk_img, bg=BG).pack(padx=16, pady=8)
        ...
        ttk.Button(btn_frame, text="💾 Сохранить .mol", command=self._save_mol).pack(
            side="left", padx=6
        )
        ttk.Button(btn_frame, text="🖼 Сохранить .png", command=self._save_png).pack(
            side="left", padx=6
        )
    ...
    def _save_mol(self):
        if not RDKIT_OK or self.rdmol is None:
            messagebox.showerror(
                "Ошибка", "RDKit не установлен или молекула недоступна."
            )
            return
...
class StructureCard(ttk.Frame):
    ...
    **kw
    ...
    def _build(self, info: dict, rdmol, idx: int):
        ...
        tk.Label(
            self,
            text=f"{formula_str}  IHD={info.get('ihd', '?')}",
            bg=PANEL,
            fg=FG,
            font=MONO,
            anchor="w",
        ...
        if rdmol is not None and PIL_OK:
            ...
            if pil:
                self._tk_img = pil_to_tk(pil)
                lbl = tk.Label(self, image=self._tk_img, bg=PANEL, cursor="hand2")
                lbl.pack(padx=4, pady=4)
                lbl.bind("<Button-1>", lambda e: self._open_detail())
        else:
            tk.Label(self, text="(нет изображения)", bg=PANEL, fg=WARN, font=MONO).pack(
                padx=4, pady=20
            )
